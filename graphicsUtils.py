...
def remove_from_screen(x, d_o_e=None):
    _canvas.delete(x)
    if d_o_e is None:
        d_o_e = _root_window.dooneevent
    d_o_e()
...
def move_to(object, x, y=None, d_o_e=None):
    if y is None:
        try:
            x, y = x
    ...
    if d_o_e is None:
        d_o_e = _root_window.dooneevent
    d_o_e()  # Process pending events


def move_by(object, x, y=None, d_o_e=None, lift=False):
    if y is None:
        try:
            x, y = x
        except (TypeError, ValueError):
